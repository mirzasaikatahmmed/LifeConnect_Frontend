...
export default function CreateBloodRequestPage() {
  ...
  const [formData, setFormData] = useState<CreateBloodRequestData>({
    bloodType: '',
    urgencyLevel: '',
    hospitalName: '',
    hospitalAddress: '',
  ...
  const handleInputChange = (field: keyof CreateBloodRequestData, value: string | number) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
    ...
    try {
      ...
      try {
        ...
        if (!response.ok && (response.status === 400 || response.status === 500)) {
          console.log('Main create endpoint failed, trying simple endpoint...');
          endpoint = `${process.env.NEXT_PUBLIC_API_URL}/api/blood-requests-simple`;
          response = await fetch(endpoint, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${authToken}`,
      ...
      const newBloodRequest = await response.json();
      console.log('Blood request created successfully:', newBloodRequest);

      // Redirect to the blood requests list
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center">
            <div>
              ...
              <p className="text-gray-600 mt-2">Create a new blood donation request</p>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="text-sm text-gray-600 space-y-1">
                <p><span className="font-medium">Blood Type:</span> {formData.bloodType || 'Not selected'}</p>
                <p><span className="font-medium">Units:</span> {formData.unitsNeeded}</p>
                <p><span className="font-medium">Urgency:</span> {formData.urgencyLevel || 'Not selected'}</p>
                <p><span className="font-medium">Hospital:</span> {formData.hospitalName || 'Not entered'}</p>
                <p><span className="font-medium">Needed By:</span> {formData.neededBy ? new Date(formData.neededBy).toLocaleString() : 'Not set'}</p>
              </div>
            </div>
        ...
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h3 className="text-blue-900 font-medium mb-2">Creating Blood Requests</h3>
          <ul className="text-blue-800 text-sm space-y-1">
            <li>• All fields marked with * are required</li>
            <li>• Choose the appropriate urgency level based on patient condition</li>
            <li>• Ensure the hospital address is complete and accurate</li>
            <li>• Set a realistic deadline for when blood is needed</li>
            <li>• The request will be visible to matching donors once created</li>
          </ul>
        </div>
      </div>
    </div>
