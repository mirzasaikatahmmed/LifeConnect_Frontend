...
//   const [user, setUser] = useState<any>(null); // localStorage থেকে parsed admin/state
...
// // 2️⃣ fetch user data when user state updates
// useEffect(() => {
...
//     axios
//       .get(`http://localhost:4000/manager/getUserDataById/${user.admin.id}`, {
...
// // 4️⃣ Original data to restore on cancel
...
// // 5️⃣ Sync formData and originalData when fetchedUser updates
// useEffect(() => {
//   if (fetchedUser) {
...
//       bloodType: fetchedUser.bloodType as ProfileData['bloodType'],
//       userType: fetchedUser.userType as ProfileData['userType'],
...
// //   email: fetchedUser?.email || '',
// //   phoneNumber: fetchedUser?.phoneNumber || '',
// //   bloodType: fetchedUser?.bloodType as ProfileData['bloodType'] || 'O+',
// //   userType: fetchedUser?.userType as ProfileData['userType'] || 'donor',
...
export default function ProfilePage() {
  ...
  // 2️⃣ Fetch user data from API when user is available
  useEffect(() => {
    if (user?.id) {
      ...
      axios
        ...
        .then((res) => {
          console.log("API Response - Fetched User Data:", res.data);
          setFetchedUser(res.data);
        })
        .catch((err) => {
          console.error("Error fetching user data:", err);
          // যদি API call fail হয় তাহলে localStorage থেকে ডেটা নিবো
          if (user?.admin) {
            console.log("Using localStorage data as fallback");
            setFetchedUser(user.admin);
          }
        })
        .finally(() => {
          setIsLoading(false);
  ...
  // 3️⃣ Update form data when fetchedUser changes
  useEffect(() => {
    if (fetchedUser) {
      console.log("Updating form data with fetched user:", fetchedUser);
  ...
  const handleSave = async() => {
    ...
    const token = user.access_token; // make sure you store token at login

    const res = await axios.patch(
      `http://localhost:4000/manager/users/${user.id}`, // your backend route
      changedData,
      { headers: { Authorization: `Bearer ${token}` } }
    );
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    <span className="text-xs text-blue-600">
                      {fetchedUser?.createdAt ? new Date(fetchedUser.createdAt).toLocaleDateString() : 'N/A'}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            ...
            <div className="text-xs text-yellow-700 space-y-1">
              ...
              <p><strong>Fetched User:</strong> {JSON.stringify(fetchedUser, null, 2)}</p>
              <p><strong>localStorage User:</strong> {JSON.stringify(user, null, 2)}</p>
            </div>
          </div>
        </div>
      )}
